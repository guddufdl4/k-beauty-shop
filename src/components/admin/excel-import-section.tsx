...
export function ExcelImportSection({
  ...
  const sectionClass = isCard
    ? "flex h-full min-h-0 flex-col overflow-hidden rounded-2xl border-2 border-rose-200 bg-white shadow-sm"
    : "space-y-4 rounded-2xl border-2 border-rose-300 bg-gradient-to-br from-rose-50 via-white to-white p-6 shadow-md ring-1 ring-rose-200";
  ...
  return (
    ...
    >
      <div className={`shrink-0 border-b border-rose-50 ${isCard ? "px-4 py-3" : ""}`}>
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <div className="mb-1.5 inline-flex items-center rounded-full bg-rose-600 px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-white">
              Excel Upload
            </div>
            <h2 className={`font-semibold text-zinc-900 ${isCard ? "text-sm" : "text-lg"}`}>
              {TITLE}
            </h2>
            {!isCard ? <p className="mt-1 text-sm text-zinc-500">{DESC}</p> : null}
          </div>
          <a
            href="/templates/hanmi-import-template.csv"
            className="text-xs font-medium text-rose-600 hover:underline sm:text-sm"
          >
            {TEMPLATE}
          </a>
        </div>
      ...
      <div className={`min-h-0 flex-1 space-y-3 overflow-auto ${isCard ? "p-4" : ""}`}>
        <form onSubmit={handleSubmit} className="flex flex-col gap-2 sm:flex-row sm:items-center">
          <input
            ...
            required
            accept=".xlsx,.xls,.csv"
            className="block w-full flex-1 rounded-lg border border-zinc-300 px-3 py-2 text-sm file:mr-3 file:rounded-md file:border-0 file:bg-zinc-100 file:px-3 file:py-1.5 file:text-sm file:font-medium"
          />
          <button
            type="submit"
            disabled={pending}
      ...
      {isCard ? (
        <div className="shrink-0 border-t border-zinc-100 px-4 py-2.5 text-[11px] text-zinc-500">
          {lastCollectedAt ? (
            <span>
              최근 수집:{" "}
