...
export default async function AdminHeroSettingsPage() {
  ...
  if (!configured) {
    return (
      <main className="mx-auto max-w-3xl px-4 py-16">
        <h1 className="text-2xl font-bold text-zinc-900">관리자 · 메인 배너</h1>
  ...
  if (!user || profile?.role !== "admin") {
    return (
      <main className="mx-auto max-w-3xl px-4 py-16">
        <h1 className="text-2xl font-bold text-zinc-900">관리자 · 메인 배너</h1>
        <p className="mt-4 text-zinc-600">관리자 계정으로 로그인해야 배너를 변경할 수 있습니다.</p>
        <Link
    ...
    <main className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
      <div className="mb-8 flex items-center justify-between gap-4">
        <div>
          ...
          <h1 className="mt-1 text-3xl font-bold text-zinc-900">메인 배너</h1>
          <p className="mt-2 text-sm text-zinc-500">
            홈페이지 상단 히어로 배너의 이미지와 문구를 변경합니다.
          </p>
        </div>
      ...
      <AdminSettingsNav />
      <AdminHeroSettingsForm initialSettings={settings} />
    </main>
  );
}